...
//     function getRaffleState(Raffle storage raffle) external view returns (string memory){
//         if (block.timestamp < raffle.open_timestamp) return "SOON";
//         else if (block.timestamp > raffle.close_timestamp) return "CLOSED";
//         else return "OPEN";
//     }

//     function getWinners(Raffle storage raffle) external view returns (address[] memory) {
//         return raffle.winners;
//     }

//     function getRandomNumbers(Raffle storage raffle) external view returns (uint[] memory) {
//         return raffle.random_numbers;
//     }

//     function getParticipants(Raffle storage raffle) external view returns (address[] memory) {
//         return raffle.participants;
//     }

//     function isOpen(Raffle storage raffle) public view returns (bool){
//         return block.timestamp >= raffle.open_timestamp && block.timestamp <= raffle.close_timestamp ? true : false;
//     }

//     function hasEnoughEntries(Raffle storage raffle) public view returns (bool){
//         return raffle.participants.length <= raffle.max_ticket;
//     }

//     function hasWon(Raffle storage raffle, address user) external view returns (bool){
...
// contract RaffleVRF is VRFConsumerBaseV2, Authorizable {
...
//     uint256 public current_winners_amount;

//     RaffleTicket raffleTicket;
//     RaffleLib.Raffle public currentRaffle;

//     using RaffleLib for RaffleLib.Raffle;

        
    
//     constructor(uint64 subscriptionId, address _raffleTicket) VRFConsumerBaseV2(vrfCoordinator){
//         COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
//         raffleTicket = RaffleTicket(_raffleTicket);
...
//     function requestRandomWords(uint raffleId, uint32 winnersAmount) external {
//         require(msg.sender == address(raffleTicket), "Only callable by raffleTicket.");
//         // Will revert if subscription is not set and funded.
//         // require(raffle.isOpen(), "Raffle is closed.");
//         // require(raffle.hasEnoughEntries(), "Not enough participants.");
//         current_raffle = raffleId;
//         winners_amount = winnersAmount;
//         uint32 numWords = winnersAmount;
...
//             currentRaffle.random_numbers.push(randomWords[i]);
//         }
//     }

//     function pickWinners(RaffleLib.Raffle memory raffle) public onlyAuthorized {
